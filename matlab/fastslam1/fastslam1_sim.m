...
h.xt= patch(0,0,'g'); % vehicle true
h.xm= patch(0,0,'r'); % mean vehicle estimate
h.obs= plot(0,0,'y'); % observations
h.xfp= plot(0,0,'r.'); % estimated features (particle means)
h.xvp= plot(0,0,'r.'); % estimated vehicle (particles)
h.cov= plot(0,0); % covariances of max weight particle
